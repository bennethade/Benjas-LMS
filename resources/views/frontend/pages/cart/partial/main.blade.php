                    ...
                    <tbody>
                        ...
                        @forelse($cart as $item)
                            <tr>
                                ...
                                <td>
                                    <a href="course-details.html" class="text-black font-weight-semi-bold">{{ $item->course->course_name }}</a>
                                    <p class="fs-14 text-gray lh-20">By 
                                        <a href="teacher-detail.html" class="text-color hover-underline">
                                            {{ $item->course->user->first_name }} {{ $item->course->user->surname }}</a>, {{ $item->course->course_title }}
                                        &amp; More!
                                    </p>
                                </td>
                                <td>
                                    <ul class="generic-list-item font-weight-semi-bold">
                                        <li class="text-black lh-18">N{{ $item->course->discount_price }}</li>
                                        <li class="before-price lh-18">N{{ $item->course->selling_price }}</li>
                                    </ul>
                                </td>

                                <td>

                                    <button type="button" class="remove-course-btn icon-element icon-element-xs shadow-sm border-0"
                                        data-id="{{ $item->id }}" data-toggle="tooltip" data-placement="top" title="Remove">
                ...
                <div class="bg-gray p-4 rounded-rounded mt-40px">
                    ...
                    <ul class="generic-list-item pb-4">
                        <li class="d-flex align-items-center justify-content-between font-weight-semi-bold">
                            ...
                            <span id="subTotalValue">N{{ $subTotal }}</span> <!-- Update ID for easy targeting -->
                        </li>
                    ...
                    <a href="{{ route('checkout.index') }}" class="btn theme-btn w-100">Checkout <i class="la la-arrow-right icon ml-1"></i></a>
                </div>
            </div>
        ...
        $('#applyCouponBtn').click(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    ('#totalDiscount').text(
                        `N${totalDiscount.toFixed(2)}`); // Show the total discount amount
                    ...
                    $('#totalAmount').text(
                        `N${totalAmount.toFixed(2)}`); // Show the updated total
